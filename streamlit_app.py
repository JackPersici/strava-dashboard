...
st.markdown(
    ...
    .hero-wrap {{ background: linear-gradient(180deg, rgba(11,24,48,0.88) 0%, rgba(8,18,35,0.88) 100%); border: 1px solid {BORDER}; border-radius: 22px; padding: 20px; margin-bottom: 16px; box-shadow: 0 10px 30px rgba(0,0,0,0.22); }}
    .app-title {{ font-size: 3rem; font-weight: 800; line-height: 1; margin-bottom: 0.4rem; letter-spacing: -0.03em; }}
    .app-subtitle {{ color: {MUTED}; font-size: 1rem; margin-bottom: 0.1rem; }}
    .filter-wrap {{ background: linear-gradient(180deg, rgba(11,24,48,0.85) 0%, rgba(8,18,35,0.85) 100%); border: 1px solid {BORDER}; border-radius: 18px; padding: 14px 14px 4px 14px; margin-bottom: 18px; box-shadow: 0 8px 24px rgba(0,0,0,0.18); }}
    .kpi-card {{ background: linear-gradient(180deg, rgba(13,27,52,1) 0%, rgba(9,22,42,1) 100%); border: 1px solid {BORDER}; border-radius: 18px; padding: 16px 18px 14px 18px; min-height: 125px; box-shadow: 0 8px 24px rgba(0,0,0,0.16); }}
    ...
    .panel {{ background: linear-gradient(180deg, rgba(11,24,48,0.98) 0%, rgba(8,18,35,0.98) 100%); border: 1px solid {BORDER}; border-radius: 18px; padding: 14px 14px 10px 14px; box-shadow: 0 8px 24px rgba(0,0,0,0.16); margin-bottom: 14px; }}
    ...
    .stMultiSelect label, .stSelectbox label, .stDateInput label, .stTextInput label, .stNumberInput label {{ color: #e5e7eb !important; font-weight: 600; }}
    .stMultiSelect div[data-baseweb="select"] *, .stSelectbox div[data-baseweb="select"] *, .stDateInput * {{ color: white !important; }}
...
def current_year_projection(df: pd.DataFrame) -> pd.DataFrame:
    ...
    proj = proj.rename(columns={group_col: "sport_grouped"})
...
def top_sports_panels(sport_df: pd.DataFrame) -> dict:
    ...
    return {
        "distance": sport_df.sort_values("distance_km", ascending=False)[[group_col, "distance_km"]].head(3).rename(columns={group_col: "sport_label"}).to_dict("records"),
        "time": sport_df.sort_values("moving_time_h", ascending=False)[[group_col, "moving_time_h"]].head(3).rename(columns={group_col: "sport_label"}).to_dict("records"),
        "elevation": sport_df.sort_values("elevation_m", ascending=False)[[group_col, "elevation_m"]].head(3).rename(columns={group_col: "sport_label"}).to_dict("records"),
    }
...
with f1:
    selected_sports = st.multiselect("Filtro sport", options=sports, default=[], placeholder="Vuoto = tutti gli sport")
with f2:
...
overview_tab, per_sport_tab, trend_tab, performance_tab, projections_tab, zone_tab = st.tabs(["Overview", "Per sport", "Trend", "Performance", "Proiezioni", "Zone"])
...
with overview_tab:
    ...
    with a:
        ...
        if not monthly_sport_df.empty:
            ...
            fig = px.bar(monthly_sport_df, x="month", y="distance_km", color="sport_grouped", barmode="stack", color_discrete_map=color_map, category_orders={"month": month_order})
    ...
    with d:
        ...
        if not cumulative_metric_df.empty:
            ...
            fig = px.line(temp, x="month_label", y="cumulative", color="year_str", markers=True, color_discrete_map=color_map, category_orders={"month_label": month_order})
    ...
    with e:
        ...
        if bp.empty:
            ...
            for _, row in bp.iterrows():
                icon = sport_icon(str(row["sport"]))
                st.markdown(f"<div class='insight-row'><div class='insight-main'>{row['value']}</div><div class='insight-sub'>{row['label']} · {row['date']} · {icon} {row['sport']}</div></div>", unsafe_allow_html=True)
        section_close()
    with f:
        ...
        if zones.empty:
            ...
            for _, row in zones.iterrows():
                st.markdown(f"<div class='insight-row'><div class='insight-main'>{row['zone']}</div><div class='insight-sub'>{row['activities_pct']:.0f}% attività · {row['time_pct']:.0f}% tempo</div></div>", unsafe_allow_html=True)
        section_close()
    ...
    with i:
        st.markdown(mini_stat_html("Sport principale", f"{sport_icon(main_sport['sport'])} {main_sport['sport']}", f"{main_sport['pct']:.0f}% della distanza"), unsafe_allow_html=True)

with per_sport_tab:
    a, b = st.columns([2.0, 0.95])
    with a:
        section_open("Riepilogo per sport")
        show = sport_summary_df.rename(columns={"sport_grouped": "Sport", "activities": "Attività", "distance_km": "Distanza (km)", "moving_time_h": "Tempo (h)", "elevation_m": "Dislivello (m)", "avg_speed_kmh": "Vel. media"}).copy()
        for col in ["Distanza (km)", "Tempo (h)", "Dislivello (m)", "Vel. media"]:
            if col in show.columns:
                show[col] = show[col].map(lambda x: f"{x:,.1f}")
        show["Sport"] = show["Sport"].map(lambda x: f"{sport_icon(str(x))} {x}")
    ...
    with b:
        ...
        for r in top_sports["distance"]:
            st.markdown(f"<div class='insight-row'><div class='insight-main'>{sport_icon(r['sport_label'])} {r['sport_label']}</div><div class='insight-sub'>{fmt_km(r['distance_km'])}</div></div>", unsafe_allow_html=True)
        ...
        for r in top_sports["time"]:
            st.markdown(f"<div class='insight-row'><div class='insight-main'>{sport_icon(r['sport_label'])} {r['sport_label']}</div><div class='insight-sub'>{fmt_h(r['moving_time_h'])}</div></div>", unsafe_allow_html=True)
        ...
        for r in top_sports["elevation"]:
            st.markdown(f"<div class='insight-row'><div class='insight-main'>{sport_icon(r['sport_label'])} {r['sport_label']}</div><div class='insight-sub'>{fmt_m(r['elevation_m'])}</div></div>", unsafe_allow_html=True)
        section_close()
...
with trend_tab:
    ...
    with a:
        ...
        if not trend_metric_df.empty:
            ...
            if temp_trend["year"].nunique() > 1:
                color_map = {str(y): "#94c5ff" for y in temp_trend["year"].dropna().unique()}
                color_map[str(distance_compare["current_year"])] = ACCENT
                fig = px.line(temp_trend, x="month", y=selected_metric, color="year_str", markers=True, line_shape="linear", category_orders={"month": month_order}, color_discrete_map=color_map)
            else:
                fig = px.line(temp_trend, x="month", y=selected_metric, markers=True, line_shape="linear", category_orders={"month": month_order})
    ...
    with b:
        ...
        fig = go.Figure(go.Indicator(mode="gauge+number", value=progress, number={"suffix": "%", "font": {"size": 42}}, gauge={"axis": {"range": [0, 100], "tickcolor": TEXT}, "bar": {"color": GREEN}, "bgcolor": "#16233f", "borderwidth": 0, "steps": [{"range": [0, 50], "color": "#1b2b4c"}, {"range": [50, 100], "color": "#233559"}] }))
    ...
    with a:
        ...
        if top5.empty:
            ...
            for idx, (_, row) in enumerate(top5.iterrows(), start=1):
                st.markdown(f"<div class='insight-row'><div class='insight-main'>{idx}. {row['distance_km']:.1f} km</div><div class='insight-sub'>{sport_icon(row['sport_grouped'])} {row['sport_grouped']} · {row['date_str']}</div></div>", unsafe_allow_html=True)
        section_close()
    with b:
    ...
    with c:
        ...
        if records.empty:
            ...
            for _, row in records.iterrows():
                st.markdown(f"<div class='insight-row'><div class='insight-main'>{row['record']}</div><div class='insight-sub'>{row['value']} · {row['date']}</div></div>", unsafe_allow_html=True)
        section_close()
...
with projections_tab:
    ...
    with a:
        ...
        if proj.empty:
            ...
            proj_show = proj[["sport_grouped", "proj_distance_km", "proj_time_h", "proj_elevation_m", "proj_activities", "delta_distance_pct", "delta_time_pct", "delta_elevation_pct", "delta_activities_pct"]].rename(columns={"sport_grouped": "Sport", "proj_distance_km": "Distanza (km)", "proj_time_h": "Tempo (h)", "proj_elevation_m": "Dislivello (m)", "proj_activities": "Attività", "delta_distance_pct": "Δ Distanza %", "delta_time_pct": "Δ Tempo %", "delta_elevation_pct": "Δ Dislivello %", "delta_activities_pct": "Δ Attività %"}).copy()
            ...
            proj_show["Sport"] = proj_show["Sport"].map(lambda x: f"{sport_icon(str(x))} {x}")
    ...
    with b:
        ...
        if not proj.empty:
            ...
            st.markdown(mini_stat_html("Probabilità di superare l'anno scorso", f"{reach_pct:.0f}%", "con il ritmo attuale"), unsafe_allow_html=True)
        else:
            st.markdown("<div class='big-empty'>Nessun dato disponibile.</div>", unsafe_allow_html=True)
        section_close()
...
with zone_tab:
    ...
    zone_colors = {"Zona 1 (Recupero)": "#60a5fa", "Zona 2 (Endurance)": "#4ade80", "Zona 3 (Tempo)": "#f59e0b", "Zona 4 (Soglia)": "#f97316", "Zona 5 (VO2 Max)": "#ef4444"}
    with a:
