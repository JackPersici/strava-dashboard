...
from app.metrics import group_small_sports, weekly_by_sport
from app.storage import load_activities
...
def _weekly_by_sport_from_data(data: dict, fallback_monthly: pd.DataFrame) -> pd.DataFrame:
    ...
    if df is None or df.empty:
        # The overview usually receives aggregated data only. For the weekly
        # toggle we need activity-level rows, so as a safe fallback we read the
        # local Streamlit cache generated by the sync step. If the file is not
        # present, the chart degrades gracefully.
        try:
            df = load_activities("data/activities_modular.parquet")
        except Exception:
            df = pd.DataFrame()
    ...
    # Keep the weekly view aligned with the current period represented by the
    # monthly aggregate passed to the overview.
    if not fallback_monthly.empty and "year" in fallback_monthly.columns and "year" in temp.columns:
        years = fallback_monthly["year"].dropna().astype(int).unique().tolist()
        if years:
            temp = temp[temp["year"].isin(years)].copy()

    if "sport_grouped" not in temp.columns:
        if "sport_label" not in temp.columns and "sport_type" in temp.columns:
            temp["sport_label"] = temp["sport_type"]
        temp = group_small_sports(temp)

    # If the user filtered to a subset of sports, the monthly aggregate already
    # reflects that subset. Mirror it in the weekly chart.
    if not fallback_monthly.empty and "sport_grouped" in fallback_monthly.columns:
        visible_sports = [str(x) for x in fallback_monthly["sport_grouped"].dropna().unique().tolist()]
        all_sports = [str(x) for x in temp.get("sport_grouped", pd.Series(dtype=str)).dropna().unique().tolist()]
        if visible_sports and len(visible_sports) < len(all_sports):
            temp = temp[temp["sport_grouped"].astype(str).isin(visible_sports)].copy()

    return weekly_by_sport(temp)
...
def _render_monthly_card(
    ...
    # Use a native Streamlit bordered container so the selectboxes remain real
    ...
    with st.container(border=True):
        ...
        if not labels:
            ...
            _render_trend_chart_fragment(fig, labels, min_width, height=188)
