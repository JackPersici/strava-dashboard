...
def plot_style(fig: go.Figure, height: int = 300, show_legend: bool = True) -> go.Figure:
    fig.update_layout(
        height=height,
        margin=dict(l=6, r=6, t=4, b=4),
        paper_bgcolor=TRANSPARENT,
        plot_bgcolor=TRANSPARENT,
        font=dict(color=TEXT, family="Inter, system-ui, -apple-system, BlinkMacSystemFont, sans-serif", size=11),
        legend=dict(
            orientation="h",
            yanchor="bottom",
            y=1.02,
        ...
        hoverlabel=dict(bgcolor="#17263C", bordercolor="rgba(255,255,255,0.10)", font=dict(color=TEXT, size=12)),
    )
    fig.update_xaxes(
        showgrid=False,
        zeroline=False,
...
def monthly_distance_chart(monthly_sport_df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_traces(marker_line_width=0, opacity=0.95, hovertemplate="%{x}<br>%{y:.1f} km<extra></extra>")
    fig.update_layout(bargap=0.34)
    return plot_style(fig, height=292)
...
def sport_donut_chart(sport_summary_df: pd.DataFrame) -> go.Figure:
    ...
    fig.update_traces(
        ...
        marker=dict(line=dict(color="#0E1A2B", width=2)),
        hovertemplate="%{label}<br>%{value:.1f} km<br>%{percent}<extra></extra>",
    )
    fig.update_layout(margin=dict(l=0, r=0, t=0, b=0), legend=dict(y=-0.05, x=0, font=dict(size=10, color=MUTED)))
    return plot_style(fig, height=248)
...
def cumulative_trend_chart(cumulative_metric_df: pd.DataFrame, current_year: int) -> go.Figure:
    ...
    color_map = {str(y): "rgba(143,161,184,0.55)" for y in temp["year"].dropna().unique()}
    ...
    fig.update_traces(line=dict(width=2.4), marker=dict(size=6), hovertemplate="%{x}<br>%{y:.1f}<extra></extra>")
    return plot_style(fig, height=292)
...
def monthly_trend_chart(trend_metric_df: pd.DataFrame, selected_metric: str, current_year: int) -> go.Figure:
    ...
    if temp["year"].nunique() > 1:
        ...
        fig = px.line(temp, x="month", y=selected_metric, markers=True, line_shape="linear", category_orders={"month": MONTH_ORDER}, labels={"month": "", selected_metric: ""})
        fig.update_traces(line=dict(color=ACCENT, width=2.6))
    fig.update_traces(marker=dict(size=6), hovertemplate="%{x}<br>%{y:.1f}<extra></extra>")
    return plot_style(fig, height=292)
...
def sport_distance_comparison_chart(sport_summary_df: pd.DataFrame) -> go.Figure:
    ...
    return plot_style(fig, height=315, show_legend=False)
...
def buckets_chart(buckets: pd.DataFrame) -> go.Figure:
    ...
    fig.update_layout(bargap=0.34)
    return plot_style(fig, height=300, show_legend=False)
    ...
    fig = go.Figure(
        go.Indicator(
            ...
            gauge={
                ...
                "borderwidth": 0,
                "steps": [{"range": [0, 50], "color": "rgba(143,161,184,0.10)"}, {"range": [50, 100], "color": "rgba(34,197,94,0.12)"}],
            },
        )
    )
    return plot_style(fig, height=245, show_legend=False)
...
def zones_pie_chart(zones: pd.DataFrame) -> go.Figure:
    ...
    fig = px.pie(zones, names="zone", values="activities", hole=0.66, color="zone", color_discrete_map=zone_colors)
    ...
    return plot_style(fig, height=290)
...
def zones_time_bar_chart(zones: pd.DataFrame) -> go.Figure:
    ...
    return plot_style(fig, height=290, show_legend=False)
